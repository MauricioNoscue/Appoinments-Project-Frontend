...
export class ReservationViewComponent implements OnInit, OnDestroy {
  ...
  async seleccionarHorario(h: Horario) {
    ...
    try {
      ...
      if (!lock.locked) {
        alert(`Ya está en uso (hasta ${lock.lockedUntil}).`);
        return;
      }

      const confirmar = confirm(`Agendar ${this.formatearHora(h.hora)}?`);
      if (!confirmar) {
      ...
      if (res.success) {
        alert(`¡Agendado! Id cita: ${res.citationId}`);
      } else {
        alert(`No se pudo agendar: ${res.reason}`);
      }
    } catch (e) {
      console.error(e);
